...
var getDate = function (date) {
    ...
    if (date instanceof Date || time.isValid()) {
        ...
        if ((hour_1 === 8 || hour_1 === 0) &&
            ...
            second_1 === 0 &&
            millisecond === 0)
            return [year_1, month_1, date_1, undefined, undefined, undefined];
        return [year_1, month_1, date_1, hour_1, minute_1, second_1];
    }
    var pattern = /(?:(\d+)[/-](\d+)[/-](\d+))?\s*(?:(\d+):(\d+)(?::(\d+))?)?/u;
    var _a = pattern.exec(date.trim()) || [], year = _a[1], month = _a[2], day = _a[3], hour = _a[4], minute = _a[5], second = _a[6];
...
var compareDate = function (dataA, dataB) {
    if (!dataA)
        return 1;
    if (!dataB)
        return -1;
...
var filterArticle = function (pages, filterFunc) {
...
var sortArticle = function (pages) {
    return pages.slice(0).sort(function (prev, next) {
        var prevSticky = prev.frontmatter.sticky;
        var nextSticky = next.frontmatter.sticky;
        var prevTime = prev.frontmatter.time || prev.frontmatter.date;
        var nextTime = next.frontmatter.time || next.frontmatter.date;
...
var generatePagination = function (pages, perPage) {
